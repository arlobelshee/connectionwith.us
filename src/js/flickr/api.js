...
const API_HOST = url.parse(
	process.env.REACT_APP_FLICKR_HOST || "https://api.flickr.com/services/rest/"
);
...
export const flickr = (() => {
	...
	if (
		...
		location.protocol === API_HOST.protocol
	) {
		return flickrXHR;
	}
...
export function streamPhotos(query, { pageSize = 10 } = {}) {
	...
	return {
		async next() {
			...
			({ page, pages, photo } = await searchPhotos(query, {
				page: page + 1,
				per_page: pageSize
			}));
